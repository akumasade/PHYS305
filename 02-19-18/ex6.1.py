...
def acceleration(mass, pos, vel, eps2):
    n = len(mass)
    acc = np.zeros((n,3))
    tau = 1e20
    for i in range(n):
        dx   = pos - pos[i]#use this
        ...
        # I guess we're calculating the time step in the fcn...? T2,T3
        #need to add the vel as parameter and a new returned value
        dv = vel-vel[i]
        vdotx = (dx*dv).sum(axis=1) + 1e-20
        dx2 = (dx**2).sum(axis=1)
        dv[i] = 1e20#so that we aren't dividing by zero
        dx2[i] = 1e20#so that zero doesn't become the minimum value
        tau1 = np.sqrt(dx2/(dv**2).sum(axis=1))
        tau2 = (dx**2).sum(axis=1)/vdotx
        a = np.sqrt((acc**2).sum(axis=1))+1e-20
        tau3 = np.sqrt(np.sqrt(dx2)/a)

        tau = np.amin(np.array([tau, tau1, tau2, tau3]))

    #return shortest tau of all pairs
    return acc, tau
...
def step(t, mass, pos, vel, eps2, dt):
    ...
    acc, tau = acceleration(mass, pos, vel, eps2)
    pos += dt*(vel+0.5*dt*acc)
    anew, tau = acceleration(mass, pos, vel, eps2)
    vel += 0.5*dt*(acc+anew)
