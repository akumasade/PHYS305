...
def energy(mass, pos, vel, eps2):
    T = np.sum(0.5*mass*(vel**2).sum(axis=1))#0.5mv^2
...
def get_acc(mass, pos, eps2):
    ...
    for i in range(n):
        dx = pos - pos[i,:]
        dr2 = (dx**2).sum(axis=1)**2
        #fix this with np.reshape()
        temp = np.array([mass[j]*d/((d**2).sum()+eps2*eps2)**3./2 for j,d in enumerate(dx)])
        acc[i,:] = temp.sum(axis=0)
    #print acc
    return acc

def step(t, mass, pos, vel, eps2, dt):
    a = get_acc(mass, pos, eps2)
    return t+dt,pos,vel
